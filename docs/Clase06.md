...
## **Persistencia con DuckDB y Despliegue en GCP**
...
```bash
git add .
git commit -m "Clase06: Persistencia con DuckDB y despliegue en GCP completada"
git push
```
...
Implementar **persistencia de estudios de Optuna con DuckDB** para permitir la continuación de optimizaciones interrumpidas, y desplegar el proyecto en **Google Cloud Platform (GCP)** utilizando SSH, TMUX y gestión de recursos en la nube.

---

## 📑 Índice de la clase

1. **Persistencia de Optuna con DuckDB** - Automatizar almacenamiento de trials en base de datos local
2. **Retomar estudios interrumpidos** - Cargar estudios existentes desde DuckDB
3. **Configuración de instancia en GCP** - Crear y configurar VM en Ubuntu
4. **Conexión SSH a GCP** - Acceso remoto seguro a la instancia
5. **Gestión de sesiones con TMUX** - Mantener procesos activos tras desconexión
6. **Monitoreo de recursos** - htop, CPU y memoria en tiempo real
7. **Ejecución remota del proyecto** - Setup completo en GCP
8. **Google Cloud Storage (Buckets)** - Almacenamiento de resultados en la nube

---

## 1) Persistencia de Optuna con DuckDB

Hasta ahora, nuestros estudios de Optuna se almacenaban solo en archivos JSON. Si el proceso se interrumpe, perdemos todo el progreso. **DuckDB** nos permite persistir el estado completo del estudio y retomarlo cuando sea necesario.

### **¿Por qué DuckDB?**

- **Ligero y rápido**: No requiere servidor separado (como PostgreSQL)
- **Archivo único**: Base de datos embebida en un solo archivo `.duckdb`
- **Compatible con Optuna**: Soporte nativo para almacenamiento de estudios
- **Portabilidad**: Fácil de mover entre máquinas (local a GCP)
- **Sin dependencias externas**: Ideal para entornos de producción

### **Actualización de config.yaml**

```yaml
# config.yaml
STUDY_NAME : "Compe_02_(V0.0)"

BUCKET_NAME: "/users/joaquintschopp/TSCHOPP-PROJECT/Compe_02"

competencia02:
    DATA_PATH: "data/competencia.csv"
    SEMILLA: [811157, 502717, 782911, 464939, 213713]
...
**Nota: Recordar modificar "config.py" para que tome los nuevos parametros.**

### **Implementación de persistencia en optimización**
...
* **Persistencia automática** de estudios de Optuna con DuckDB
* **Capacidad de retomar** optimizaciones interrumpidas sin perder progreso
* **Conexión SSH** establecida y configurada
* **Dominio de TMUX** para gestionar sesiones remotas persistentes
* **Monitoreo de recursos** con htop y comandos del sistema
* **Proyecto ejecutándose en GCP** con entorno virtual y dependencias
* **Sincronización con GitHub** desde instancia remota

---

**¡Éxito en el despliegue en la nube! 🚀**
