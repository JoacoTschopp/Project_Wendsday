...
def guardar_predicciones_finales(
    resultados_df: pd.DataFrame, nombre_archivo=None
) -> str:
    """
    ...
    if not all(col in resultados_df.columns for col in ["numero_cliente", "predict"]):
        raise ValueError(
            "El DataFrame debe contener las columnas 'numero_cliente' y 'predict'"
        )
    ...
    if not all(val in [0, 1] for val in valores_unicos):
        raise ValueError(
            f"Los valores de 'predict' deben ser 0 o 1. Encontrados: {valores_unicos}"
        )
    ...
    logger.info("Formato del archivo:")
