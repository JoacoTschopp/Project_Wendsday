...
from .config import MES_TEST, SEMILLA, STUDY_NAME
...
def crear_grafico_ganancia_test(
    y_true: np.ndarray, y_pred_proba: np.ndarray, ganancias_acumuladas: np.ndarray
) -> str:
    """
    ...
    plt.plot(
        x_filtrado, y_filtrado, color="blue", linewidth=2.5, label="Ganancia Acumulada"
    )
    ...
    plt.title(
        f"Ganancia Acumulada por Orden de Predicción (Filtrada) - {STUDY_NAME}",
        fontsize=14,
        fontweight="bold",
    )
    ...
    logger.info(
        f"  - Porcentaje sobre umbral óptimo: {corte_optimo/len(y_pred_proba)*100:.1f}%"
    )
    logger.info(
        f"  - Porcentaje sobre umbral fijo: {corte_fijo/len(y_pred_proba)*100:.1f}%"
    )
    logger.info(
        f"  - Diferencia: {corte_fijo - corte_optimo:,} clientes más con umbral fijo"
    )
    logger.info(
        f"  - Puntos mostrados: {len(x_filtrado)} de {len(ganancias_acumuladas)} totales"
    )

    return ruta_archivo


def crear_grafico_comparativo_multiple_semillas(
    y_true: np.ndarray, resultados_por_semilla: List[Dict[str, Any]]
) -> str:
    """
    ...
    todas_las_curvas: List[np.ndarray] = []
    ...
    ganancia_maxima_global = max(
        resultado["ganancia_maxima"] for resultado in resultados_por_semilla
    )
    ...
    for resultado in resultados_por_semilla:
        ...
        plt.scatter(
            indice_maximo, ganancia_maxima, color=color, s=80, zorder=5, alpha=0.9
        )

        # Agregar línea vertical para el corte óptimo de esta semilla
        plt.axvline(
            x=indice_maximo, color=color, linestyle="--", alpha=0.5, linewidth=1
        )

    # Calcular la curva media
    # Primero, asegurar que todas las curvas tengan la misma longitud
    curvas_normalizadas: List[np.ndarray] = []
    ...
    plt.axvline(
        x=indice_maximo_media, color="red", linestyle="-", alpha=0.7, linewidth=1.5
    )

    # Agregar anotación con el número de clientes en el punto máximo
    plt.annotate(
        f"Máximo: {ganancia_maxima_media:,.0f}\n{indice_maximo_media:,} clientes",
    ...
    plt.ylabel("Ganancia Acumulada", fontsize=12)
    plt.title(
        f"Comparación de Ganancia Acumulada por Semilla - {STUDY_NAME}",
        fontsize=16,
        fontweight="bold",
    )
    plt.grid(True, alpha=0.3)
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ruta_archivo = (
        f"resultados/{STUDY_NAME}_grafico_comparativo_semillas_{timestamp}.jpg"
    )
    ...
    indices_maximos = [
        resultado["indice_maximo"] for resultado in resultados_por_semilla
    ]

    logger.info("Estadísticas comparativas del gráfico:")
    ...
    logger.info(
        f"  - Desviación estándar de ganancias: {np.std(ganancias_maximas):,.0f}"
    )
    logger.info(f"  - Corte óptimo promedio: {np.mean(indices_maximos):,.0f} clientes")
    logger.info(
        f"  - Desviación estándar de cortes: {np.std(indices_maximos):,.0f} clientes"
    )
    logger.info(
        f"  - Umbral de filtrado: {umbral_ganancia:,.0f} (66% del máximo global)"
    )
    logger.info(
        f"  - CURVA MEDIA: Ganancia máxima: {ganancia_maxima_media:,.0f} en {indice_maximo_media:,} clientes"
    )

    for resultado in resultados_por_semilla:
        logger.info(
            f"  - Semilla {resultado['semilla']}: Max={resultado['ganancia_maxima']:,.0f}, Corte={resultado['indice_maximo']:,}"
        )
        mlflow.log_metric(
            "ganancia_maxima", resultado["ganancia_maxima"], step=resultado["semilla"]
        )
...
def generar_grafico_test_completo(
    df: pd.DataFrame,
    mejores_params: Optional[Dict[str, Any]] = None,
    tiradas: int = 20,
    ...
    """
    logger.info(
        "=== INICIANDO GENERACIÓN DE GRÁFICO DE TEST CON MÚLTIPLES SEMILLAS ==="
    )
    ...
    semillas: np.ndarray = _generar_semillas_primas(tiradas, SEMILLA[0])
    ...
    for i, semilla in enumerate(semillas):  # Usar las primeras 5 semillas
        ...
        logger.info(
            f"  - Ganancia máxima con semilla {semilla}: {ganancia_maxima:,.0f}"
        )

    # Crear gráfico comparativo
    ruta_grafico = crear_grafico_comparativo_multiple_semillas(
        y_true, resultados_por_semilla
    )
