...
from .config import FINAL_PREDIC, FINAL_TRAIN, SEMILLA
...
def preparar_datos_entrenamiento_final(df: pd.DataFrame) -> tuple:
    ...
    logger.info("Datos preparados para entrenamiento final")
...
def entrenar_modelo_final(
    ...
    if undersampling < 1.0:
        logger.info(
            f"Aplicando undersampling (ratio={undersampling}) en entrenamiento final"
        )
        df_train_final = aplicar_undersampling(
            df_train_final, undersampling, random_state=semilla
        )
...
def generar_predicciones_finales(
    ...
    resultados = pd.DataFrame(
        {"numero_cliente": clientes_predict, "predict": predicciones_binary}
    )
    ...
    logger.info(
        f"  Predicciones positivas: {predicciones_positivas:,} ({porcentaje_positivas:.2f}%)"
    )
    logger.info(
        f"  Predicciones negativas: {total_predicciones - predicciones_positivas:,}"
    )
